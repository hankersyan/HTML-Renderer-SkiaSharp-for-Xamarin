...
namespace TheArtOfDev.HtmlRenderer.XamarinForms.Adapters
{
    ...
    internal sealed class GraphicsAdapter : RGraphics
    {
        ...
        public GraphicsAdapter(SKCanvas g, bool useGdiPlusTextRendering, bool releaseGraphics = false)
            : base(MonoAdapter.Instance, Utils.Convert(g.LocalClipBounds))
        {
        ...
        public override void PopClip()
        {
            ...
            //SKRect rect = Utils.Convert2SKRect(_clipStack.Peek());
            //_g.ClipRect(rect);
        }
        ...
        public override void PushClip(RRect rect)
        {
            ...
            //_g.ClipRect(Utils.Convert2SKRect(rect));
        }
        ...
        public override void PushClipExclude(RRect rect)
        {
            ...
            _g.ClipRect(Utils.Convert2SKRect(rect), SKClipOperation.Difference);
        }
        ...
        public override RSize MeasureString(string str, RFont font, RFontStyle style)
        {
            if (_useGdiPlusTextRendering)
            {
                bool isWhitespace = str != null && str.IndexOf('\n') < 0 && string.IsNullOrWhiteSpace(str);
                if (isWhitespace)
                {
                    str = "".PadLeft(str.Length, '&');
                    //str = "&";
                }
                ...
                SKPaint textPaint = new SKPaint();// { TextSize = (float)realFont.FontSize };
                textPaint.TextSize = (float)font.Size;
                textPaint.Typeface = HtmlContainer.DefaultTypeface;
                if ((style & RFontStyle.Italic) > 0)
                    textPaint.TextSkewX = -0.3F; //italic
                if ((style & RFontStyle.Bold) > 0)
                    textPaint.FakeBoldText = true; //Bold
                ...
                if (font.Height < 0 && realFont != null)
                {
                    var height = (int)realFont.FontSize;// realFont.Height;
                    var descent = realFont.FontSize; //realFont.FontSize * realFont.FontFamily.GetCellDescent(realFont.Style) / realFont.FontFamily.GetEmHeight(realFont.Style);
#if !MONO
                    fontAdapter.SetMetrics(height, (int)Math.Round((height - descent + .5f)));
#else
                    fontAdapter.SetMetrics(height, (int)Math.Round(descent + 1f));//(int)Math.Round((height - descent + 1f))
#endif
                }

                return Utils.Convert2RSize(size);
            }
            else
            {
#if !MONO
        ...
        public override void MeasureString(string str, RFont font, double maxWidth, out int charFit, out double charFitWidth)
        {
            ...
            if (_useGdiPlusTextRendering)
            {
                ...
                var size = MeasureString(str, font, RFontStyle.Regular);

                for (int i = 1; i <= str.Length; i++)
                {
                    charFit = i - 1;
                    RSize pSize = MeasureString(str.Substring(0, i), font, RFontStyle.Regular);
        ...
        public override void DrawString(string str, RFont font, RColor color, RPoint point, RSize size, bool rtl, RFontStyle style)
        {
            if (_useGdiPlusTextRendering)
            {
                ReleaseHdc();
                ...
                if ((style & RFontStyle.Bold) > 0)
                    textPaint.FakeBoldText = true; //Bold
                SKPoint pt = Utils.Convert2SKPoint(point);
                pt.Y += (float)size.Height;
                _g.DrawText(str, pt, textPaint);
            }
            else
            {
#if !MONO
        ...
        public override void DrawLine(RPen pen, double x1, double y1, double x2, double y2)
        {
            ...
            _g.DrawLine((float)x1, (float)y1, (float)x2, (float)y2, new SKPaint() { Color = ((PenAdapter)pen).Pen.Color, StrokeWidth = (float)pen.Width });
        }
        ...
        public override void DrawRectangle(RPen pen, double x, double y, double width, double height)
        {
            ...
            ZPen pen2 = ((PenAdapter)pen).Pen;
            _g.DrawRect((float)x, (float)y, (float)width, (float)height, new SKPaint() { Color = pen2.Color });
        }
        ...
        public override void DrawRectangle(RBrush brush, double x, double y, double width, double height)
        {
            ...
            ZSolidBrush zbrush = (ZSolidBrush)((BrushAdapter)brush).Brush;
            if (zbrush != null)
                _g.DrawRect((float)x, (float)y, (float)width, (float)height, new SKPaint() { Color = Utils.Convert2SKColor(zbrush.color) });
        }
        ...
        public override void DrawImage(RImage image, RRect destRect, RRect srcRect)
        {
            ...
            _g.DrawImage(((ImageAdapter)image).Image, Utils.Convert2SKRect(srcRect), Utils.Convert2SKRect(destRect));
        }
        ...
        public override void DrawImage(RImage image, RRect destRect)
        {
            ...
            _g.DrawImage(((ImageAdapter)image).Image, Utils.Convert2SKRect(destRect));
        }
