    ...
    internal class CssBox : CssBoxProperties, IDisposable
    {
        ...
        internal virtual void MeasureWordsSize(RGraphics g)
        {
            if (!_wordsSizeMeasured)
            {
                ...
                if (Words.Count > 0)
                {
                    RFontStyle style = RFontStyle.Regular;
                    if (string.Compare(this.FontStyle, "italic", true) == 0)
                        style |= RFontStyle.Italic;
                    if (string.Compare(this.FontWeight, "bolder", true) == 0 || string.Compare(this.FontWeight, "bold", true) == 0)
                        style |= RFontStyle.Bold;

                    foreach (var boxWord in Words)
                    {
                        boxWord.Width = boxWord.Text != "\n" ? g.MeasureString(boxWord.Text, ActualFont, style).Width : 0;
                        boxWord.Height = ActualFont.Height > 0 ? ActualFont.Height : g.MeasureString("&", ActualFont, style).Height;
                    }
                }
        ...
        protected virtual void PaintImp(RGraphics g)
        {
            ...
            {
                var clipped = !isInTd() && RenderUtils.ClipGraphicsByOverflow(g, this);
        ...
        private bool isInTd()
        {
            CssBox parent = this.ParentBox;
            bool isParentTd = false;
            while (parent != null)
            {
                if (parent.HtmlTag != null)
                {
                    isParentTd = string.Compare(parent.HtmlTag.Name, "td", true) == 0;
                }
                if (isParentTd)
                    break;
                parent = parent.ParentBox;
                        ...
                        if (clip != RRect.Empty)
                        {
                            ...
                            if (word.Selected)
                            {
                                ...
                                if (HtmlContainer.SelectionForeColor != RColor.Empty && (word.SelectedStartOffset > 0 || word.SelectedEndIndexOffset > -1))
                                {
                                    ...
                                    g.DrawString(word.Text, ActualFont, ActualColor, wordPoint, new RSize(word.Width, word.Height), isRtl, style);
                                    g.PopClip();
                                    g.PushClip(rect);
                                    g.DrawString(word.Text, ActualFont, GetSelectionForeBrush(), wordPoint, new RSize(word.Width, word.Height), isRtl, style);
                                    g.PopClip();
                                }
                                else
                                {
                            ...
                            else
                            {
                                ...
                                RFont font = ActualFont;
                                RColor color = ActualColor;

                                if (isInTd())
                                {
                                    wordPoint.X -= this.ParentBox.ActualBorderRightWidth;
                                    wordPoint.Y -= this.ParentBox.ActualBorderBottomWidth;
                                }

                                g.DrawString(str, font, color, wordPoint, new RSize(word.Width, word.Height), isRtl, style);
                            }
                        }
                    }
                }
        ...
        protected void PaintDecoration(RGraphics g, RRect rectangle, bool isFirst, bool isLast)
        {
            ...
            {
                y = rectangle.Bottom - 2; //Math.Round(rectangle.Top + ActualFont.UnderlineOffset);
            }
